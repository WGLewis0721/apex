...
def connect_screen(t):
    ...
    txt(d,(36,100),'A few details. Everything connected.',29,'#25252f',True)
    txt(d,(36,145),'Your billing provider. Your plans. Your product.',14,'#9898a5')
    rr(d,(35,192,485,423),'#f8f8fc',15,'#e7e7f0');txt(d,(58,214),'PAYMENT PROVIDER',11,'#8e8e9d',True)
    rr(d,(58,249,462,313),'white',10,'#e2e2ed');txt(d,(78,266),'stripe',23,'#635bff',True)
    ready=t>2.5
    txt(d,(434,280),'Connected' if ready else 'Connect account',13,'#07866a' if ready else '#777784',anchor='rm')
    if ready: check(d,342,273)
    txt(d,(58,340),'Subscriptions and payment events',14,'#51515e');txt(d,(58,369),'Ready for your customer experience.',12,'#9696a5')
    rr(d,(513,192,975,423),'#f8f8fc',15,'#e7e7f0');txt(d,(536,214),'PRODUCT PLAN',11,'#8e8e9d',True)
    txt(d,(538,252),'Starter',22,'#282833',True);txt(d,(951,261),'$29 / month',19,'#282833',True,'rm')
    d.line((537,294,951,294),fill='#e2e2ed');txt(d,(538,315),'AI tokens included',14,'#777785');txt(d,(951,315),'1,000',17,'#635bff',True,'ra')
    button(d,(536,365,951,403),'Configuration ready' if t>4 else 'Save plan','#635bff',13)
    return im
